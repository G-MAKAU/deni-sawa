  ...
  const load = React.useCallback(
    async (targetPage: number, statusFilter: string, dateFrom: string, dateTo: string) => {
      ...
      try {
        ...
        const { entries: rows, pagination } = await adminFetch<{ entries: LogEntry[]; pagination: { total: number } }>(
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <input
              ...
              className="h-9 rounded-lg border border-[var(--a-border)] bg-[var(--a-card)] px-2 text-sm focus:border-[#E8510A] focus:outline-none"
            />
