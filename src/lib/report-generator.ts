...
export async function resolveProviderConfig(provider: ReportProvider): Promise<ProviderConfig & { label: string }> {
  ...
  if (configuredType && s.AI_API_KEY) {
    ...
    const model =
      s.AI_MODEL ||
      envModelFor(type) ||
      (type === 'google' ? 'gemini-flash-latest' : type === 'anthropic' ? 'claude-sonnet-4-5' : 'gpt-4o-mini');
    return {
      type,
  ...
  if (provider === 'google') {
    ...
    return {
      type: 'google',
      apiKey,
      model: s.GEMINI_MODEL ?? process.env.GEMINI_MODEL ?? 'gemini-flash-latest',
      label: 'Gemini',
    };
  }
  if (provider === 'openrouter' || provider === 'openai') {
    ...
    const model =
      ...
      (provider === 'openrouter' ? 'google/gemini-flash-latest' : 'gpt-4o-mini');
